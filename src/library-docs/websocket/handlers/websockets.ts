...
import { ConnectionMetadata, EventMessage, WebSocketError, WebSocketResponse, BroadcastOptions } from '../types/websocket';
import { docClient } from '../../clients/aws';
...
export const defaultHandler: APIGatewayProxyWebsocketHandlerV2 = async (event) => {
  if (!event.body) {
    return createResponse(400, createError(400, 'Missing message body'));
  }
  ...
  try {
    ...
    return createResponse(200, { message: 'Message received', data: message, timestamp: new Date().toISOString() });
  } catch (error) {
